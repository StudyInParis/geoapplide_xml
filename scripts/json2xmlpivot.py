...
#! coding: utf-8
"""
Prend en Entrée les fichiers json convertis en xml et les formate au format xml_pivot
INSTALLER xml.etree
"""
...
if __name__ == '__main__':
	...
	dic=getInfos("../XML/fichiers_2xml/"+fic, arrond="arrondissement", nom="nom_etablissement", adresse="adresse", coordinates="coordonnees")
	print(fic)
	writeInFile(dic, "cinema", "../XML/fichiers_2xml_pivot/"+fic[:-4]+"_pivot.xml")
	
	fic="liste_des_marches_de_quartier_a_paris.xml"
	dic=getInfos("../XML/fichiers_2xml/"+fic, arrond="arrondissement", nom="marche", adresse="adresse_complete_poi_approchant", coordinates="geo_coordinates")
	print(fic)
	writeInFile(dic, "marche", "../XML/fichiers_2xml_pivot/"+fic[:-4]+"_pivot.xml")

	fic = "liste-des-cafes-a-un-euro.xml"
	dic=getInfos("../XML/fichiers_2xml/"+fic, arrond="arrondissement", nom="nom_du_cafe", adresse="adresse", coordinates="geoloc")
	print(fic)
	writeInFile(dic,"cafe", "../XML/fichiers_2xml_pivot/"+fic[:-4]+"_pivot.xml")

	fic="distributeurspreservatifsmasculinsparis2012.xml"
	dic=getInfos("../XML/fichiers_2xml/"+fic, arrond="arrond", nom="site", adresse="adresse", coordinates="xy")
	print(fic)
	writeInFile(dic,"preservatifs", "../XML/fichiers_2xml_pivot/"+fic[:-4]+"_pivot.xml")

	fic="restauration_france_entiere.xml"
	dic=getInfosCrous("../XML/fichiers_2xml/"+fic)
	print(fic)
